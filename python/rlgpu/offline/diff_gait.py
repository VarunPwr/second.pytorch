...
class DifferentiableGaitLib(nn.Module):
    def __init__(self, num_gaits=4, num_legs=4, dim=3, init_scale=CONTACT_FORCE_SCALE):
        ...
        self.params = torch.Tensor(num_gaits, dim).uniform_(-1, 1).to("cuda:0")
    ...
    def forward(self, logits):
        ...
        gaits = self.params.unsqueeze(0).unsqueeze(0).repeat(
            logits.shape[0], logits.shape[1], 1, 1) * hard_prob.unsqueeze(-1)
        gaits = gaits.sum(-2)
        return gaits
