...
class XarmBimanual(BaseTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.xarm_default_dof_pos = to_torch(
            [1.157, -1.066, -0.155, -2.239, -1.841, 1.003, 0.469, 0.035, 0.035], device=self.device)
        self.dof_state = gymtorch.wrap_tensor(dof_state_tensor)
        print(self.dof_state.shape)
        self.xarm_dof_state = self.dof_state.view(
            self.num_envs, -1, 2)[:, :self.num_xarm_dofs]
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if "asset" in self.cfg["env"]:
            ...
            xarm_asset_file = self.cfg["env"]["asset"].get(
                "assetFileNamexarm", xarm_asset_file)
        ...
        xarm_asset = self.gym.load_asset(
            self.sim, asset_root, xarm_asset_file, asset_options)
        ...
        self.xarm_dof_lower_limits = to_torch(
            self.xarm_dof_lower_limits, device=self.device)
        self.xarm_dof_upper_limits = to_torch(
            self.xarm_dof_upper_limits, device=self.device)
        self.xarm_dof_speed_scales = torch.ones_like(
            self.xarm_dof_lower_limits)
        ...
        xarm_start_pose.p = gymapi.Vec3(1.0, 0.0, 0.0)
        ...
        max_agg_bodies = num_xarm_bodies + \
            num_cabinet_bodies + self.num_props * num_prop_bodies
        max_agg_shapes = num_xarm_shapes + \
            num_cabinet_shapes + self.num_props * num_prop_shapes
        ...
        for i in range(self.num_envs):
            ...
            xarm_actor = self.gym.create_actor(
                env_ptr, xarm_asset, xarm_start_pose, "xarm", i, 1, 0)
            self.gym.set_actor_dof_properties(
                env_ptr, xarm_actor, xarm_dof_props)
    ...
    def init_data(self):
        hand = self.gym.find_actor_rigid_body_handle(
            self.envs[0], self.xarms[0], "panda_link7")
        lfinger = self.gym.find_actor_rigid_body_handle(
            self.envs[0], self.xarms[0], "panda_leftfinger")
        rfinger = self.gym.find_actor_rigid_body_handle(
            self.envs[0], self.xarms[0], "panda_rightfinger")
        ...
        xarm_local_grasp_pose.p += gymapi.Vec3(
        ...
        self.xarm_local_grasp_pos = to_torch([xarm_local_grasp_pose.p.x, xarm_local_grasp_pose.p.y,
                                                xarm_local_grasp_pose.p.z], device=self.device).repeat((self.num_envs, 1))
        self.xarm_local_grasp_rot = to_torch([xarm_local_grasp_pose.r.x, xarm_local_grasp_pose.r.y,
                                                xarm_local_grasp_pose.r.z, xarm_local_grasp_pose.r.w], device=self.device).repeat((self.num_envs, 1))
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:] = compute_xarm_reward(
            ...
            self.xarm_grasp_pos, self.drawer_grasp_pos, self.xarm_grasp_rot, self.drawer_grasp_rot,
    ...
    def compute_observations(self):
        ...
        self.xarm_grasp_rot[:], self.xarm_grasp_pos[:], self.drawer_grasp_rot[:], self.drawer_grasp_pos[:] = \
        ...
        self.xarm_lfinger_pos = self.rigid_body_states[:,
                                                         self.lfinger_handle][:, 0:3]
        self.xarm_rfinger_pos = self.rigid_body_states[:,
                                                         self.rfinger_handle][:, 0:3]
        self.xarm_lfinger_rot = self.rigid_body_states[:,
                                                         self.lfinger_handle][:, 3:7]
        self.xarm_rfinger_rot = self.rigid_body_states[:,
        ...
        to_target = self.drawer_grasp_pos - self.xarm_grasp_pos
    ...
    def reset(self, env_ids):
        ...
        # reset xarm
        pos = tensor_clamp(
            self.xarm_default_dof_pos.unsqueeze(
                0) + 0.25 * (torch.rand((len(env_ids), self.num_xarm_dofs), device=self.device) - 0.5),
            self.xarm_dof_lower_limits, self.xarm_dof_upper_limits)
        self.xarm_dof_pos[env_ids, :] = pos
        self.xarm_dof_vel[env_ids, :] = torch.zeros_like(
            self.xarm_dof_vel[env_ids])
    ...
    def pre_physics_step(self, actions):
        ...
        targets = self.xarm_dof_targets[:, :self.num_xarm_dofs] + \
            self.xarm_dof_speed_scales * self.dt * self.actions * self.action_scale
    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                px = (self.xarm_grasp_pos[i] + quat_apply(self.xarm_grasp_rot[i], to_torch(
                    [1, 0, 0], device=self.device) * 0.2)).cpu().numpy()
                py = (self.xarm_grasp_pos[i] + quat_apply(self.xarm_grasp_rot[i], to_torch(
                    [0, 1, 0], device=self.device) * 0.2)).cpu().numpy()
                pz = (self.xarm_grasp_pos[i] + quat_apply(self.xarm_grasp_rot[i], to_torch(
                ...
                px = (self.xarm_lfinger_pos[i] + quat_apply(self.xarm_lfinger_rot[i], to_torch(
                    [1, 0, 0], device=self.device) * 0.2)).cpu().numpy()
                py = (self.xarm_lfinger_pos[i] + quat_apply(self.xarm_lfinger_rot[i], to_torch(
                    [0, 1, 0], device=self.device) * 0.2)).cpu().numpy()
                pz = (self.xarm_lfinger_pos[i] + quat_apply(self.xarm_lfinger_rot[i], to_torch(
                    [0, 0, 1], device=self.device) * 0.2)).cpu().numpy()
                ...
                px = (self.xarm_rfinger_pos[i] + quat_apply(self.xarm_rfinger_rot[i], to_torch(
                    [1, 0, 0], device=self.device) * 0.2)).cpu().numpy()
                py = (self.xarm_rfinger_pos[i] + quat_apply(self.xarm_rfinger_rot[i], to_torch(
                    [0, 1, 0], device=self.device) * 0.2)).cpu().numpy()
                pz = (self.xarm_rfinger_pos[i] + quat_apply(self.xarm_rfinger_rot[i], to_torch(
                    [0, 0, 1], device=self.device) * 0.2)).cpu().numpy()
    ...
    def _update_viewer(self):
        if self.viewer is not None:
            lookat = self.rigid_body_states[:, self.hand_handle][0, 0:3]
            p = [_lookat - _camera_distance for _camera_distance,
                 _lookat in zip([0, 2, -1], lookat)]
...
def compute_xarm_reward(
    ...
    xarm_grasp_pos, drawer_grasp_pos, xarm_grasp_rot, drawer_grasp_rot,
    ...
    d = torch.norm(xarm_grasp_pos - drawer_grasp_pos, p=2, dim=-1)
    ...
    around_handle_reward = torch.where(xarm_lfinger_pos[:, 2] > drawer_grasp_pos[:, 2],
                                       torch.where(xarm_rfinger_pos[:, 2] < drawer_grasp_pos[:, 2],
                                                   around_handle_reward + 0.5, around_handle_reward), around_handle_reward)
    # reward for distance of each finger from the drawer
    finger_dist_reward = torch.zeros_like(rot_reward)
    lfinger_dist = torch.abs(xarm_lfinger_pos[:, 2] - drawer_grasp_pos[:, 2])
    rfinger_dist = torch.abs(xarm_rfinger_pos[:, 2] - drawer_grasp_pos[:, 2])
    finger_dist_reward = torch.where(xarm_lfinger_pos[:, 2] > drawer_grasp_pos[:, 2],
                                     torch.where(xarm_rfinger_pos[:, 2] < drawer_grasp_pos[:, 2],
    ...
    rewards = torch.where(xarm_lfinger_pos[:, 0] < drawer_grasp_pos[:, 0] - distX_offset,
                          torch.ones_like(rewards) * -1, rewards)
    rewards = torch.where(xarm_rfinger_pos[:, 0] < drawer_grasp_pos[:, 0] - distX_offset,
                          torch.ones_like(rewards) * -1, rewards)

    reset_buf = torch.where(xarm_lfinger_pos[:, 0] < drawer_grasp_pos[:, 0] - distX_offset,
                            torch.ones_like(reset_buf), reset_buf)
    reset_buf = torch.where(xarm_rfinger_pos[:, 0] < drawer_grasp_pos[:, 0] - distX_offset,
                            torch.ones_like(reset_buf), reset_buf)
...
def compute_grasp_transforms(hand_rot, hand_pos, xarm_local_grasp_rot, xarm_local_grasp_pos,
    ...
    return global_xarm_rot, global_xarm_pos, global_drawer_rot, global_drawer_pos
